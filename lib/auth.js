...
async function createSession(res, userId) {
  ...
  const sessions = pruneExpired(await store.read('sessions'));
  sessions.push({
    token,
    userId,
  ...
  await store.write('sessions', sessions);
...
async function destroySession(req, res) {
  ...
  if (token) {
    const sessions = (await store.read('sessions')).filter((session) => session.token !== token);
    await store.write('sessions', sessions);
  }
...
async function getCurrentUser(req) {
  ...
  const session = (await store.read('sessions')).find((entry) => entry.token === token);
  if (!session || new Date(session.expiresAt).getTime() <= Date.now()) {
    return null;
  }
  return (await store.read('users')).find((user) => user.id === session.userId) || null;
}
