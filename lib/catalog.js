...
function normalizeShades(shades) {
  ...
  return shades.map((shade) => {
    if (typeof shade === 'string') {
      return { name: shade, label: shade, color: '#d9a08b', images: [], tintPhotos: false, stock: 0, soldOut: true };
    }
    ...
    return { name: String(shade || '').trim() || 'Shade', label: String(shade || '').trim() || 'Shade', color: '#d9a08b', images: [], tintPhotos: false, stock: 0, soldOut: true };
  });
}
