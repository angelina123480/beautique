...
function sendWithResend(message) {
  ...
  const fromAddress = process.env.EMAIL_FROM || 'Beautique <hello@lunabeauty.com>';
...
function sendEmail(type, email, data) {
  ...
  const history = store.read('emailLog');
  history.push({
    id: Date.now(),
    type: message.type,
    to: message.to,
  ...
  return sendWithResend(message).then(() => message);
}
