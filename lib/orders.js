...
/** Used by /checkout/success to check whether a Stripe session was already
    turned into an order, so a page refresh doesn't create a duplicate. */
...
function createOrderQueries(order) {
  const queries = [sql`
    insert into orders (id, user_id, user_email, status, subtotal, discount, discount_code,
      shipping, total, points_earned, payment_method, address, created_at, stripe_session_id)
    values (${order.id}, ${order.userId}, ${order.userEmail}, ${order.status}, ${order.subtotal},
      ...
      ${order.paymentMethod}, ${order.address}, ${order.createdAt}, ${order.stripeSessionId || null})
  `];
