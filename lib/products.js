...
async function replaceShades(productId, shades) {
  ...
  for (let i = 0; i < (shades || []).length; i++) {
    const s = shades[i];
    await sql`
      insert into product_shades (product_id, name, label, color, images, tint_photos, sort_order, stock)
      values (${productId}, ${s.name}, ${s.label || s.name}, ${s.color || '#d9a08b'}, ${s.images || []}, ${Boolean(s.tintPhotos)}, ${i}, ${Math.max(0, Math.floor(Number(s.stock) || 0))})
    `;
  }
}
...
async function createProduct(product) {
  await sql`
    insert into products (id, name, brand, price, sale_price, badge, emoji, category, tone,
      description, stock, sold_out, images, model_image, scent_family, skin_goals, wink_map)
    values (${product.id}, ${product.name}, ${product.brand}, ${product.price}, ${product.salePrice},
      ...
      ${product.stock}, ${product.soldOut}, ${product.images || []}, ${product.modelImage || ''},
...
async function saveProduct(product) {
  await sql`
    update products set
      ...
      sold_out = ${product.soldOut}, images = ${product.images || []}, model_image = ${product.modelImage || ''},
    ...
    if (existing.length) {
      await sql`
        update products set
          ...
          model_image = ${p.modelImage || ''}, scent_family = ${p.scentFamily || []},
      ...
      await sql`
        ...
        values (${p.id}, ${p.name}, ${p.brand || ''}, ${p.price || 0}, ${p.salePrice ?? null},
          ...
          ${p.stock || 0}, ${Boolean(p.soldOut)}, ${p.images || []}, ${p.modelImage || ''},
          ${p.scentFamily || []}, ${p.skinGoals || []}, ${JSON.stringify(p.winkMap || {})})
      `;
    }
    if (p.shades !== undefined) {
      /* The repo's data/products.json never carries per-shade stock (that's
         real inventory, not catalog metadata), so merge in whatever stock
         each shade already has in the database — otherwise a sync would
         silently zero out real stock for every shaded product. */
