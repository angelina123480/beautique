...
async function updateUser(id, patch) {
  ...
  const rows = await sql.query(`update users set ${setClause} where id = $1 returning *`, [id, ...values]);
  return mapUser(rows[0], await getDiscountCodes(id));
}
...
async function setDiscountCodeUsed(userId, code, usedAt) {
  await sql`update user_discount_codes set used_at = ${usedAt}
