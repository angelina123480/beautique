...
/* orders.user_id has no ON DELETE action (unlike sessions/user_discount_codes,
   which cascade) — deleting a user who ever placed an order would otherwise
   fail with a foreign-key violation. Detach their orders first so order
   history (revenue, past purchases) survives the account being removed. */
async function deleteUser(id) {
  await sql`update orders set user_id = null where user_id = ${id}`;
